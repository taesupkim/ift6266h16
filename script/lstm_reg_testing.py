...
def set_seeding_function(generator_model):
...
# set generator input data list
    ...
    # set seeding inputs
    seeding_inputs  = [input_sequence, ]

    # set seeding outputs
    seeding_outputs = [data_hidden, data_cell]

    # set seeding function
    seeding_function = theano.function(inputs=seeding_inputs,
                                       outputs=seeding_outputs,
...
def set_sampling_function(generator_model):
    ...
    seed_input_data = tensor.matrix(name='seed_input_data',
                                    dtype=floatX)
    seed_hidden_data = tensor.matrix(name='seed_hidden_data',
                                     dtype=floatX)
    seed_cell_data = tensor.matrix(name='seed_cell_data',
                                    dtype=floatX)
    ...
    # get generator output data
    generator_output = generator_model[0].seeded_forward(generator_input_data_list)
