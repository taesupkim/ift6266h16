...
def set_tf_update_function(input_emb_param,
    ...
    output_emb_sequence = output_data_set[0]
    output_sequence     = tensor.dot(output_emb_sequence, tensor.transpose(input_emb_param))
    ...
    tf_updates_dict = get_model_and_params_updates(layers=generator_model,
                                                   params=[input_emb_param,],
...
def set_sample_function(input_emb_param,
    ...
    sample_emb_data = output_data_set[0]
    sample_data     = tensor.dot(sample_emb_data, tensor.transpose(input_emb_param))
...
if __name__=="__main__":
    ...
    model_name = 'LSTM_GAN_HIDDEN_FF(TRUNC_LINEAR_EMB_ADAGRAD)' \
                + '_FEATURE{}'.format(int(feature_size)) \
                + '_HIDDEN{}'.format(int(hidden_size)) \

    # generator model
    generator_model = set_generator_model(input_size=hidden_size,
                                          hidden_size=hidden_size)

    # discriminator model
    discriminator_feature_model = set_discriminator_feature_model(hidden_size=hidden_size,
                                                                  feature_size=400)
    discriminator_output_model = set_discriminator_output_model(feature_size=400)

    # set optimizer
    tf_generator_optimizer      = RmsProp(learning_rate=0.001).update_params
    gan_generator_optimizer     = RmsProp(learning_rate=0.001).update_paramss
